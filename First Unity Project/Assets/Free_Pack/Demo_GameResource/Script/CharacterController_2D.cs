...
public class CharacterController_2D : MonoBehaviour {
    ...
    void Move_Fuc()
    {
        ...
        //grounded = Physics2D.IsTouchingLayers(myCollider, whatIsGround); //can return true or false (checks if a collider is touching another collider)

        if (Input.GetKeyDown(KeyCode.Space) || Input.GetMouseButtonDown(0)) //Either the player presses space bar or clicks (can be used for tapping on a screen for mobile)
            {
            //m_rigidbody.velocity = new Vector3(0, 0, 0);
